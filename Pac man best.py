...
class Enemy(object):
    ...
    def set_path(self, bx, by, ex, ey, level, typ, stepped, key=[], ax=0, ay=0):
        if stepped:
            if typ == 0:
...
def drawWindow(move_count, cherry_timer, score, lives, prev_spr):
...
def drawStartWindow():
    game_start = title_font.render("PAC MAN", 5, YELLOW)
    text = font.render("[PRESS ANY KEY TO BEGIN]", 5, WHITE)
    text2 = small_font.render("Created by Kevin, Modified by 13519075", 5, WHITE)
...
def drawWinWindow():
    ...
    text = font.render("SCORE: "+str(score+add_score+(lives+1)*1000), 5, WHITE)
    text2 = font.render("LIVES: "+str(lives+1)+" (+"+str((lives+1)*1000)+" score)", 5, WHITE)
    pygame.draw.rect(screen, BLACK, (16, 16, 16*20, 16*25))
...
def drawLoseWindow():
    ...
    text = font.render("SCORE: "+str(score+add_score+(lives+1)*1000), 5, WHITE)
    text2 = font.render("LIVES: "+str(lives+1), 5, WHITE)
    pygame.draw.rect(screen, BLACK, (16, 16, 16*20, 16*25))
...
pygame.display.set_caption("Pac Man modified by 13519075")
...
while not exit_flag:
    ...
    if start_flag:
        ...
        if path == []:
            path = enemy[0].set_path(enemy[0].rect.x//16, enemy[0].rect.y//16, player.rect.x//16, player.rect.y//16, level, 0, stepped)
        if path1 == []:
            path1 = enemy[1].set_path(enemy[1].rect.x//16, enemy[1].rect.y//16, player.rect.x//16, player.rect.y//16, level, 1, stepped)
        if path2 == []:
            path2 = enemy[2].set_path(enemy[2].rect.x//16, enemy[2].rect.y//16, player.rect.x//16, player.rect.y//16, level, 2, stepped, prev_key)
        if path3 == []:
            path3 = enemy[3].set_path(enemy[3].rect.x//16, enemy[3].rect.y//16, player.rect.x//16, player.rect.y//16, level, 3, stepped, prev_key, enemy[0].rect.x//16, enemy[0].rect.y//16)
        enemy[0].move(path)
        enemy[1].move(path1)
        enemy[2].move(path2)
        enemy[3].move(path3)
    ...
    if start_flag:
        if lives < 0:
            ...
            prev_spr = drawWindow(move_count, cherry_timer, score, lives, prev_spr)
    else:
        drawStartWindow()
    pygame.display.flip()
    pygame.time.delay(1)
